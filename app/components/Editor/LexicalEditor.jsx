...
export default function LexicalEditor({ onContentSave, onClose }) {
  const [editor] = useLexicalComposerContext();

  const handleSaveClick = () => {
    editor.update(() => {
      const root = $getRoot();
      const content = root.getTextContent(); // kan anpassas till JSON eller plain text
      if (onContentSave) {
        onContentSave(content); // Anropa callback med content
      }
      if (onClose) {
        onClose(); // Stäng editorn efter sparning
      }
    });
  }
